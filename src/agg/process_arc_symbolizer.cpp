...
{
    render_arc_symbolizer(double rx, double ry, Rasterizer & ras, Renderer & ren, Common & common, ProjTransform const& prj_trans)
        : ras_(ras),
          ren_(ren),
          common_(common),
                                  ...
                                  proj_transform const& prj_trans)
{
    ...
    using render_arc_symbolizer_type = detail::render_arc_symbolizer<rasterizer, renderer_type, renderer_common, proj_transform>;
    render_arc_symbolizer_type apply(rx, ry, *ras_ptr, ren, common_, prj_trans);
    mapnik::util::apply_visitor(geometry::vertex_processor<render_arc_symbolizer_type>(apply), feature.get_geometry());
}

template void agg_renderer<image_rgba8>::process(arc_symbolizer const&,
                                              mapnik::feature_impl &,
